  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_340px]">
        <Card><CardHeader className="border-b"><CardTitle>Contract checks</CardTitle></CardHeader><CardContent className="divide-y p-0">{report.checks?.map((check) => <div className="flex gap-3 px-5 py-4 sm:px-6" key={check.id}>{check.passed ? <Check className="mt-0.5 size-4 shrink-0 text-success" /> : <CircleX className="mt-0.5 size-4 shrink-0 text-destructive" />}<div><p className="text-sm font-medium capitalize">{check.id.replaceAll("_", " ")}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{check.details}</p></div></div>) ?? <p className="p-6 text-sm text-muted-foreground">No contract report was produced.</p>}</CardContent></Card>
