...
const stateClasses: Record<RiderStationState, string> = {
  ...
  unavailable: "border-destructive/25 bg-destructive/10 text-destructive-content",
...
export default async function StatusPage({ searchParams }: StatusPageProps) {
  ...
  const visibleStations = accessibility
    ? [...accessibility.stations]
        ...
        .sort((a, b) => {
          const priority = { unavailable: 0, unknown: 1, limited: 2, accessible: 3 };
          return priority[a.state] - priority[b.state] || a.corridorOrder - b.corridorOrder;
        })
    : [];
        ...
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-3 text-base leading-7 text-muted-foreground">
              See which stations have step-free access and which elevators are working.
            </p>
          </div>
          {accessibility && (
        ...
        {!accessibility ? (
          <Card className="mt-8 overflow-hidden">
            <CardHeader className="border-b bg-muted/30">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <CardTitle className="flex items-center gap-2"><CircleHelp aria-hidden="true" className="size-4 text-muted-foreground" /> Status unavailable</CardTitle>
                  <p className="mt-1 text-sm text-muted-foreground">No verified accessibility update is available right now.</p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <span className="inline-flex items-center gap-2 rounded-full border bg-card px-3 py-1.5 text-xs font-medium text-muted-foreground"><RefreshCw aria-hidden="true" className="size-3.5" /> Waiting for an update</span>
                  <PublicRefreshButton />
                </div>
              </div>
            </CardHeader>
            <CardContent className="py-10 sm:py-14">
              <div className="mx-auto max-w-lg text-center">
                ...
                <p className="mt-2 text-sm leading-6 text-muted-foreground">
                  For immediate travel decisions, check the official SFMTA elevator page.
                </p>
                <a className="mt-5 inline-flex items-center gap-2 text-sm font-medium text-primary underline-offset-4 hover:underline" href="https://www.sfmta.com/elevator-status/elevatorstatus.php?src=prod" rel="noreferrer" target="_blank">
                  Open official SFMTA status <span className="sr-only">(opens in a new tab)</span> <ExternalLink aria-hidden="true" className="size-3.5" />
                </a>
              </div>
            </CardContent>
          </Card>
          ...
          <>
            {accessibility.trust.state === "older" && (
              <div aria-live="polite" className="mt-8 flex flex-col gap-3 rounded-xl border border-warning/30 bg-warning/10 p-4 text-sm sm:flex-row sm:items-center sm:justify-between" role="status">
            ...
            <div className="mt-8 grid grid-cols-2 gap-3 lg:grid-cols-4">
              {([
                ...
                <Card className="p-4 sm:p-5" key={state}>
                  <p className="text-2xl font-semibold tabular-nums">{accessibility.counts[state]}</p>
            ...
            >
              ...
              <label className="text-sm font-medium" htmlFor="view">
                Show stations
                <Select className="mt-2" defaultValue={view} id="view" name="view">
                  ...
                  <option value="unavailable">No confirmed step-free access</option>
                  <option value="unknown">Access not confirmed</option>
                </Select>
              </label>
              <button className={cn(buttonVariants(), "w-full md:w-auto")} type="submit">
                Show
              </button>
              {(query || view !== "all") && (
                <Link className={cn(buttonVariants({ variant: "ghost" }), "w-full md:w-auto")} href="/status">
                  Clear
                </Link>
              )}
            </form>
            ...
            <div className="mt-8 space-y-3">
              {visibleStations.length === 0 && (
                <Card className="p-8 text-center">
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">Try another station name or show all station states.</p>
                </Card>
              )}
                  ...
                  >
                    <summary className="flex cursor-pointer list-none items-center justify-between gap-4 p-4 sm:p-5 [&::-webkit-details-marker]:hidden">
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-xs text-muted-foreground">{station.elevators.length} elevator{station.elevators.length === 1 ? "" : "s"}</p>
                      </div>
                      <span className="flex shrink-0 items-center gap-2">
                        <span className={cn("rounded-full border px-3 py-1.5 text-xs font-medium", stateClasses[station.state])}>
                          {riderStateLabel(station.state)}
                        </span>
                        <ChevronDown aria-hidden="true" className="size-4 text-muted-foreground transition-transform duration-150 group-open:rotate-180" />
                      </span>
                    </summary>
                    <div className="border-t px-4 py-2 sm:px-5">
                      {[...station.elevators]
                        .sort((a, b) => Number(a.state === "working") - Number(b.state === "working"))
                        .map((elevator) => (
                          <div
                          ...
                          >
                            <div className="min-w-0">
                              ...
                              <p className="mt-1 text-xs text-muted-foreground">
                                {elevator.role ?? "Elevator access"} · Last changed{" "}
                                {formatPacific(elevator.lastChangedAt)}
                              </p>
                            ...
                            <span className={cn("inline-flex w-fit items-center gap-2 rounded-full border px-2.5 py-1 text-xs font-medium", elevator.state === "working" ? "border-success/25 bg-success/10 text-success-foreground" : elevator.state === "out_of_service" ? "border-destructive/25 bg-destructive/10 text-destructive-content" : "bg-muted text-muted-foreground")}>
                              {elevator.state === "working" ? <CircleCheck aria-hidden="true" className="size-3.5" /> : <CircleAlert aria-hidden="true" className="size-3.5" />}
            ...
            <div className="mt-8 flex flex-col gap-3 rounded-xl border bg-muted/30 p-4 text-sm sm:flex-row sm:items-center sm:justify-between">
              <p className="text-muted-foreground">For urgent travel decisions, confirm with SFMTA.</p>
              <div className="flex flex-col gap-2 sm:flex-row">
                ...
                <a
                  ...
                  target="_blank"
                >
                  Open SFMTA status <span className="sr-only">(opens in a new tab)</span> <ExternalLink aria-hidden="true" className="size-3.5" />
                </a>
              </div>
            </div>
          </>
