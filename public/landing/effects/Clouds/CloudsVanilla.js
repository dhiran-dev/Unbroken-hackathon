...
export function createClouds(elements, options = {}) {
    ...
    function syncCanvasSize() {
        ...
        if (cw > 0 && ch > 0) {
            const wpx = `${cw}px`;
            const hpx = `${ch}px`;
            if (output.style.width !== wpx)
                output.style.width = wpx;
            if (output.style.height !== hpx)
                output.style.height = hpx;
        }
    ...
    function render(delta) {
        ...
        gl.uniform2f(field.uniforms.uOffset, content.scrollLeft / Math.max(content.clientWidth, 1), -content.scrollTop / Math.max(content.clientHeight, 1));
        ...
        gl.uniform2f(composite.uniforms.uShadowShift, -config.shadowOffsetX / Math.max(output.clientWidth, 1), config.shadowOffsetY / Math.max(output.clientHeight, 1));
